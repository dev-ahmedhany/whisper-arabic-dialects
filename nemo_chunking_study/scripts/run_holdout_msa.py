"""NeMo top-strategies on real-human held-out MSA Arabic dataset.
Targets clips ≥ MIN_DUR seconds so chunking has actual room to act."""
import os, re, json, time, unicodedata, argparse
import numpy as np, jiwer, sherpa_onnx
from datasets import load_dataset

PER_CLIP = 100; SR = 16000; MIN_DUR = 6.0
...
ap.add_argument('--text-field', default='cleaned_text')
ap.add_argument('--min-dur', type=float, default=6.0)
ap.add_argument('--out', default='/tmp/results/holdout_msa.jsonl')
args = ap.parse_args()
...
print(f'--- cache {args.dataset_path} text-field={args.text_field} min-dur={args.min_dur}s ---', flush=True)
ds = load_dataset(args.dataset_path, split=args.split, streaming=True,
                  token=os.environ.get('HF_TOKEN'), trust_remote_code=True)
clips = []; skipped = 0; scanned = 0
for row in ds:
    scanned += 1
    if len(clips) >= PER_CLIP: break
    if scanned > 5000: break  # safety cap
    audio = row.get('audio')
    text = row.get(args.text_field) or row.get('text') or row.get('cleaned_text') or row.get('transcription') or ''
    if not audio or not text: skipped += 1; continue
    arr = np.asarray(audio['array'], dtype='float32')
    samples, _ = to16k(arr, audio['sampling_rate'])
    dur = len(samples) / SR
    if dur < args.min_dur: skipped += 1; continue
    clips.append({'samples': samples, 'ref': norm(text), 'dur': dur})

print(f'cached {len(clips)} clips (scanned {scanned}, skipped {skipped})', flush=True)
if not clips: exit(1)
durs = [c['dur'] for c in clips]
print(f'  clip dur: min={min(durs):.1f}s, median={sorted(durs)[len(durs)//2]:.1f}s, max={max(durs):.1f}s', flush=True)

# Show first clip comparison
samples = clips[0]['samples']
text, _ = transcribe(samples)
print(f'\nfirst clip ref:  {clips[0]["ref"][:100]}')
print(f'first clip nemo: {text[:100]}\n')
...
for name, fn in strategies:
    ...
    res = {'dataset': args.dataset_path, 'strategy': name, 'wer': wer,
           'rtf': decode_s/audio_s, 'n_clips': len(refs), 'min_dur_s': args.min_dur}
