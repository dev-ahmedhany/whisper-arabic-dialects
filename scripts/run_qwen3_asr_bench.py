"""Benchmark Qwen3-ASR-1.7B on Arabic dialect test sets.

Qwen3-ASR is an Apache-2.0 ASR model from Alibaba (Jan 2026, arxiv:2601.21337)
built on the Qwen3-Omni audio backbone. 30 languages including Arabic, but no
MSA/dialect breakdown is published — this script supplies the missing
per-dialect numbers on the same test sets the rest of the paper uses.

GPU-only (no CT2/GGUF support yet — see deploy/06_v3_data_and_eval.md §7
discussion for why). Output schema matches src.eval_harness so rows land in
runs/results.jsonl alongside Whisper rows; tagged backend=qwen3-asr in extra.
"""
...
def main() -> None:
    ...
    p.add_argument("--language", default="Arabic",
                   help="Qwen3-ASR language hint (None => auto-detect)")
    ...
    if args.model_name is None:
        args.model_name = args.model.rstrip("/").split("/")[-1].lower().replace("-", "_")
    ...
    for i, s in enumerate(samples):
        try:
            ...
            results = model.transcribe(audio=s["audio"], language=args.language)
    ...
    print(f"=== {args.dialect}: WER={wer:.4f}  CI=[{wer_lo:.4f}, {wer_hi:.4f}]  RTF={elapsed/total_audio:.3f}",
          flush=True)
