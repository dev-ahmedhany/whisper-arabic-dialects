"""Bench NVIDIA NeMo ASR (FastConformer) on Arabic test sets via NeMo native CPU.

Usable for nvidia/stt_ar_fastconformer_hybrid_large_pc_v1.0 and similar
NeMo hybrid Transducer/CTC models. Switches to CTC decoding by default.
...
def main() -> None:
    ...
    p.add_argument("--model", default="nvidia/stt_ar_fastconformer_hybrid_large_pc_v1.0")
    p.add_argument("--model-name", default="nvidia-fastconformer-ar-pc")
    ...
    p.add_argument("--decoding", default="ctc", choices=["ctc", "rnnt"])
    ...
    m = nemo_asr.models.ASRModel.from_pretrained(args.model)
    m.train(False)  # equivalent to .eval() for PyTorch modules
    m = m.cpu()
    if hasattr(m, "change_decoding_strategy") and args.decoding == "ctc":
        try:
    ...
    for i, s in enumerate(samples):
        try:
            ...
            out = m.transcribe([s["audio"]], batch_size=1, verbose=False)
            text = out[0].text if hasattr(out[0], "text") else (
                out[0] if isinstance(out[0], str) else str(out[0]))
    ...
    result = EvalResult(
        config=asdict(EvalConfig(
            ...
            compute_type="fp32", beam_size=1, cpu_threads=8, device="cpu",
        )),
        ...
        extra={"backend": "nemo-cpu", "decoding": args.decoding},
    )
